...
Kduxs stores accept the `plugins` option that exposes hooks for each mutation. A Kduxs plugin is simply a function that receives the store as the only argument:
...
Kduxs comes with a logger plugin for common debugging usage:
...
The logger file can also be included directly via a `<script>` tag, and will expose the `createKduxsLogger` function globally.
