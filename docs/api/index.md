...
- type: `Object | Function`
  ...
  The root state object for the Kduxs store. [Details](../guide/state.md)
...
- default: `false`
  ...
  Force the Kduxs store into strict mode. In strict mode any mutations to Kduxs state outside of mutation handlers will throw an Error.
...
- type: `boolean`
  ...
  Turn the devtools on or off for a particular Kduxs instance. For instance, passing `false` tells the Kduxs store to not subscribe to devtools plugin. Useful when you have multiple stores on a single page.
...
-  `subscribe(handler: Function, options?: Object): Function`
  ...
  The `subscribe` method will return an `unsubscribe` function, which should be called when the subscription is no longer needed. For example, you might subscribe to a Kduxs Module and unsubscribe when you unregister the module. Or you might call `subscribe` from inside a Kdu Component and then destroy the component later. In these cases, you should remember to unsubscribe the subscription manually.
...
-  `subscribeAction(handler: Function, options?: Object): Function`
  ...
  The `subscribe` method will return an `unsubscribe` function, which should be called when the subscription is no longer needed. For example, when unregistering a Kduxs module or before destroying a Kdu component.
  ...
  The `subscribeAction` method will return an `unsubscribe` function, which should be called when the subscription is no longer needed. For example, you might subscribe to a Kduxs Module and unsubscribe when you unregister the module. Or you might call `subscribeAction` from inside a Kdu Component and then destroy the component later. In these cases, you should remember to unsubscribe the subscription manually.
...
-  `mapState(namespace?: string, map: Array<string> | Object<string | function>): Object`
  ...
  Create component computed options that return the sub tree of the Kduxs store. [Details](../guide/state.md#the-mapstate-helper)
...
- `useStore<S = any>(injectKey?: InjectionKey<Store<S>> | string): Store<S>;`
  ...
  TypeScript users can use an injection key to retrieve a typed store. In order for this to work, you must define the injection key and pass it along with the store when installing the store instance to the Kdu app.

  First, declare the injection key using Kdu's `InjectionKey` interface.
